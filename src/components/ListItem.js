...
const ListItem = ({ item, todoData, setTodoData }) => {
  ...
  const handleDeleteClick = _id => {
    ...
    // 백엔드 데이터와 연결할 때는 삭제된 데이터를 보내고 나서 바뀐 데이터를 다시 받고 화면에 바뀐 내용을 출력해야 함
  ...
  const handleEditChange = _e => {
    setEditTitle(_e.target.value);
  };
  ...
  if (isEdit) {
    // 편집중
    return (
      <div className="flex items-center justify-between w-full mb-2 px-4 py-1 text-blue-600 bg-blue-100 border rounded">
        <div className="items-center w-3/5">
          <input
            className="w-full px-3 py-2 mr-3 text-blue-500 rounded"
            type="text"
